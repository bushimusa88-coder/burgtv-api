...
async function registerDeviceHandler(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const deviceData = {
      mac_address,
      device_type,
      device_name: device_name || `${device_type.charAt(0).toUpperCase() + device_type.slice(1)} Device`,
      m3u_url,
    ...
    const accessToken = Buffer.from(`${newDevice.id}:${newDevice.mac_address}:${Date.now()}`).toString('base64');
